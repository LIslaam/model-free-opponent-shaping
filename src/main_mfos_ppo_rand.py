"""
Same as M-FOS PPO, but training on a randomly generated game which is reset at every episode. Evaluate on a game from environments.py.
"""
import torch
...
if __name__ == "__main__":
    ...
    env = MetaGames(batch_size, opponent=args.opponent, game="random", mmapg_id=args.mamaml_id)
    memory = Memory()
    ...
    for i_episode in range(1, max_episodes + 1):
        # creating new environment every episode
        env = MetaGames(batch_size, opponent=args.opponent, game="random", mmapg_id=args.mamaml_id)
    ...
    with open(os.path.join(name, f"out_{i_episode}.json"), "w") as f:
        json.dump(rew_means, f)
    print(f"SAVING! {i_episode}")
    print(f"TRAINING COMPLETE, TESTING ON {args.game}")

    ######## Freezing policy, testing on game (e.g: test if M-FOS shows extortion on IPD)

    if not os.path.isdir(name+"_test_"+args.game.lower()):
        os.mkdir(name+"_test_"+args.game.lower())
        with open(os.path.join(name+"_test_"+args.game.lower(), "commandline_args.txt"), "w") as f:
            json.dump(args.__dict__, f, indent=2)

    # Creating new environment to use with trained agent
    ...
    for i_episode in range(num_tests + 1):
        state = env.reset()
        ...
        if i_episode % save_freq == 0:
            ppo.save(os.path.join(name+"_test_"+args.game.lower(), f"{i_episode}.pth"))
            with open(os.path.join(name+"_test_"+args.game.lower(), f"out_{i_episode}.json"), "w") as f:
                json.dump(rew_means, f)
            print(f"SAVING! {i_episode}")
