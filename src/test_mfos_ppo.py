...
if __name__ == "__main__":
    ...
    print(f"RUNNING NAME: {'runs/' + name + '/test_' + args.game}")
    if not os.path.isdir('runs/' + name + '/test_' + args.game):
        os.mkdir('runs/' + name + '/test_' + args.game)
        with open(os.path.join('runs/' + name + '/test_' + args.game, 
    ...
    directory = "runs/" + name + '/'
    checkpoint_path = directory + "{}.pth".format(32768)# max_episodes)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if i_episode % save_freq == 0:
            ppo.save(os.path.join('runs/' + name + '/test_' + args.game, f"{i_episode}.pth"))
            with open(os.path.join('runs/' + name + '/test_' + args.game, f"out_{i_episode}.json"), "w") as f:
